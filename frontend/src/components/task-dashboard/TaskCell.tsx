 ...
 */
export const TaskCell: React.FC<Props> = ({ col, task, updateTaskField, onOpenOptionDropdown, rowIdx, colIdx: _colIdx, editableCols, totalRows, focusCell }) => {
  if (col === 'done') {
    return (
        ...
        {task.statuses && task.statuses.length > 0 ? (
          task.statuses.map(s => (
            <span
              ...
              className="badge"
              style={{
                    background: rgbaFromHex(s.color, 0.2),
                    borderColor: rgbaFromHex(s.color, 0.45),
                    color: readableTextColor(s.color)
                  }}
            >
              {s.value}
            </span>
